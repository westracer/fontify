...
class Transform {
  ...
  static List<Transform> parse(String string) {
    ...
    final transforms = _transformRegExp
      .allMatches(string)
      .map((m) {
        ...
        final parameterMatches = _transformParameterRegExp.allMatches(parameterString);
        final parameterList = parameterMatches.map((m) => double.parse(m.group(0))).toList();
    ...
    switch (type) {
      ...
      case TransformType.rotate:
        ...
        if (parameterList.length > 1) {
          ...
          final t = _getTranslateMatrix(x, y)..multiply(transform)..multiply(_getTranslateMatrix(-x, -y));
          transform = t;
        }
